...
begin: PREPARE_SCHEDULE_ARRAY
	...
	begin
		if(backEndReady_i && ((i == freeEntry0) || (i == freeEntry1) || (i == freeEntry2) || (i == freeEntry3)))
			issueqSchedule_tmp[i] = 1'b1; // BUG_12: Inserted logic_bug bug
		else
...
begin: preparing_request_vector_for_FU0
	integer k;
	for(k=0; k<`SIZE_ISSUEQ; k=k+1) // BUG_9: Inserted bitwise_corruption bug
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] & ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] != 2'b00)) ? 1'b1:1'b0; // BUG_11: Inserted logic_bug bug
	end
end
