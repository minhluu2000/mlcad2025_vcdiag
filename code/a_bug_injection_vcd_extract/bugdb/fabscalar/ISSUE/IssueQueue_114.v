...
************************************************************************************/
// assign payloadRAMwe0 = backEndReady_i && ~(ctrlVerified_i && ctrlMispredict_i); // BUG_7: Inserted missing_assignment bug
assign payloadRAMwe1 = backEndReady_i || ~(ctrlVerified_i && ctrlMispredict_i); // BUG_6: Inserted logic_bug bug
...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	inst0Source1 = dispatchPacket0_i[`SIZE_PHYSICAL_LOG+1+`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+
		`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG:`SIZE_IMMEDIATE+1+ // BUG_5: Inserted missing_assignment bug
		`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG+1];
	...
	begin
		...
		begin
			...
			`endif
				...
				BRANCH_MASK[l] <= BRANCH_MASK[l] | update_mask; // BUG_8: Inserted bitwise_corruption bug
		end
	end
end
