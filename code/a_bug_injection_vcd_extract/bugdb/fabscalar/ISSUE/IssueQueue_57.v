...
assign payloadRAMDataWr1 = {inst1Source2[`SIZE_PHYSICAL_LOG:1],inst2Source1[`SIZE_PHYSICAL_LOG:1], // BUG_3: Inserted wrong_assignment bug
...
begin: PREPARE_VALID_ARRAY_MISPRED
	...
	for(i=0; i<`SIZE_ISSUEQ; i=i+1)
	begin
		if(ctrlVerified_i || ctrlMispredict_i)    /* Unnecessary logic? */ // BUG_1: Inserted logic_bug bug
		begin
...
begin: preparing_request_vector_for_FU1
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector1[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b01)) ? 1'b1:1'b0; // BUG_4: Inserted logic_bug bug
	end
end
...
begin: preparing_request_vector_for_FU3
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector3[k] = (ISSUEQ_VALID[k] & ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b11)) ? 1'b0:1'b1; // BUG_2: Inserted wrong_assignment bug
	end
end
