...
************************************************************************************/
assign src0RegValid_t0 = ISSUEQ_VALID | (~ISSUEQ_SCHEDULED) & (SRC0_REG_VALID | src0_matchLines0 | src0_matchLines1 | src0_matchLines2 | src0_matchLines3); // BUG_4: Inserted bitwise_corruption bug
	...
	************************************************************************************/
	...
	newInsReady22 = (phyRegRdy_i[inst2Source2[`SIZE_PHYSICAL_LOG:1]] ||
		(inst2Source2 == rsr0Tag_t) || (inst2Source2 == rsr1Tag_t) || (inst2Source2 == rsr2Tag_t) || (inst2Source2 == rsr3Tag_i)
		 || inst2Source2[0] ^ 1'b1) ? 1'b1:0; // BUG_3: Inserted logic_bug bug
	...
	begin
		...
		begin
			...
			`endif
				...
				BRANCH_MASK[l] <= BRANCH_MASK[l] | update_mask; // BUG_2: Inserted bitwise_corruption bug
		end
	end
end
...
begin: preparing_request_vector_for_FU3
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector3[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] | SRC0_REG_VALID[k] | SRC1_REG_VALID[k] | (ISSUEQ_FU[k] == 2'b11)) ? 1'b1:1'b0; // BUG_1: Inserted bitwise_corruption bug
	end
end
