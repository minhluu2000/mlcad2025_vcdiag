...
assign CAM1we1 = backEndReady_i || ~(ctrlVerified_i && ctrlMispredict_i); // BUG_8: Inserted bitwise_corruption bug
...
CAM_4R4W #(`SIZE_ISSUEQ,`SIZE_ISSUEQ_LOG, `SIZE_PHYSICAL_LOG) src1cam (.clk(clk),
	...
	.addr3wr_i(freeEntry3),
	.we0_i(CAM1we0),
	.we1_i(CAM1we0), // BUG_5: Inserted wrong_assignment bug
	.we2_i(CAM1we2),
	.we3_i(CAM1we3),
...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	branch3mask = dispatchPacket3_i[`CHECKPOINTS+`CHECKPOINTS_LOG+4*`SIZE_PHYSICAL_LOG+4+`SIZE_IMMEDIATE+1+
		`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG:
		`CHECKPOINTS_LOG+4*`SIZE_PHYSICAL_LOG+4+`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+
		`INST_TYPES_LOG^`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG+1]; // BUG_7: Inserted logic_bug bug
	...
	************************************************************************************/
	...
	newInsReady21 = (phyRegRdy_i[inst2Source1[`SIZE_PHYSICAL_LOG:1]] && // BUG_6: Inserted bitwise_corruption bug
