...
assign CAM0we0 = backEndReady_i && ~(ctrlMispredict_i && ctrlMispredict_i); // BUG_4: Inserted wrong_assignment bug
...
begin: preparing_request_vector_for_FU1
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector1[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b01)) ? 1'b1:1'b0; // BUG_2: Inserted logic_bug bug
	end
end
...
always @(*)
begin: preparing_request_vector_for_FU2 // BUG_1: Inserted logic_bug bug
	integer k;
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
...
Select select2(.clk(new_clk_signal), // BUG_3: Inserted wrong_assignment bug
	.reset(reset),
