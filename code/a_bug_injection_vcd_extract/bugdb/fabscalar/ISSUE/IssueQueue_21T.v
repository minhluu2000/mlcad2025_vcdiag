...
begin: UPDATE_SRC_READY_BIT
   ...
   for (j = 0; j < `SIZE_ISSUEQ; j = j + 1) begin
      if (backEndReady_i && (j == freeEntry0)) begin
        src0RegValid_t1[j] = newInsReady02;
        src1RegValid_t1[j] = newInsReady01;
      end else if (backEndReady_i && (j == freeEntry1)) begin
        src0RegValid_t1[j] = newInsReady12;
        src1RegValid_t1[j] = newInsReady11;
      end else if (backEndReady_i && (j == freeEntry2)) begin
        src0RegValid_t1[j] = newInsReady22;
        src1RegValid_t1[j] = newInsReady21;
      end else if (backEndReady_i && (j == freeEntry3)) begin
