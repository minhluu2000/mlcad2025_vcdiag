...
assign CAM0we0 = backEndReady_i || (ctrlVerified_i || ctrlMispredict_i); // BUG_4: Inserted bitwise_corruption bug
...
begin: preparing_request_vector_for_FU3
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector3[k] = (ISSUEQ_VALID[k] & ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] != 2'b11)) ? 1'b1:1'b0; // BUG_3: Inserted logic_bug bug
	end
end
