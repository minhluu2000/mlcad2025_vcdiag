...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	branch1mask = dispatchPacket1_i[`CHECKPOINTS+`CHECKPOINTS_LOG+4*`SIZE_PHYSICAL_LOG+4+`SIZE_IMMEDIATE+1+
		`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG:
		`CHECKPOINTS_LOG&4*`SIZE_PHYSICAL_LOG&4&`SIZE_IMMEDIATE&1&`LDST_TYPES_LOG& // BUG_1: Inserted bitwise_corruption bug
		`INST_TYPES_LOG+`SIZE_CTI_LOG+2*`SIZE_PC+`SIZE_CTI_LOG+1]; // BUG_3: Inserted wrong_assignment bug
...
begin: preparing_request_vector_for_FU0
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] ^ SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b00)) ? 1'b1:1'b0; // BUG_4: Inserted logic_bug bug
	end
end
