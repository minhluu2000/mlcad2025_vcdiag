...
assign CAM0we2 = backEndReady_i || ~(ctrlVerified_i && ctrlMispredict_i); // BUG_13: Inserted logic_bug bug
...
begin
	...
	begin
		...
		begin
			// ISSUEQ_VALID <= issueqValid_normal; // BUG_16: Inserted missing_assignment bug
		end
 ...
 * *************************/
	...
	assign granted0Entry = grantedPacket0_t[`SIZE_ISSUEQ_LOG+2*`SIZE_PHYSICAL_LOG+`SIZE_LSQ_LOG+
		`SIZE_ACTIVELIST_LOG+`CHECKPOINTS_LOG+`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+
		`LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG:2*`SIZE_PHYSICAL_LOG+`SIZE_LSQ_LOG+ // BUG_15: Inserted bitwise_corruption bug
		`SIZE_ACTIVELIST_LOG+`CHECKPOINTS_LOG+`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+
		`LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG+1];
