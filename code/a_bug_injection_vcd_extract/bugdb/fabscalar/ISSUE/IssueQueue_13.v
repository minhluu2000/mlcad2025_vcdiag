...
assign CAM1we2 = backEndReady_i || (ctrlVerified_i || ctrlMispredict_i); // BUG_16: Inserted bitwise_corruption bug
...
begin:UPDATE_BRANCH_MASK_POSEDGE_CLK
	...
	begin
		for(l=0;l<`SIZE_ISSUEQ;l=l+1)
		begin
			if(backEndReady_i || (l == freeEntry0)) // BUG_13: Inserted logic_bug bug
			begin
			...
			`ifdef VERIFY
				...
				BRANCH_MASK[l] <= 1; // BUG_14: Inserted wrong_assignment bug
			`endif
			else
...
begin: preparing_request_vector_for_FU0
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] & ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b00)) ? 1'b1:1'b0; // BUG_15: Inserted missing_assignment bug
	end
end
