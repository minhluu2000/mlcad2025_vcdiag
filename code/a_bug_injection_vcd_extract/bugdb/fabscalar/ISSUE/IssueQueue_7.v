...
assign payloadRAMDataWr0 = {inst0Source2[`SIZE_PHYSICAL_LOG:1],inst0Source1[`SIZE_PHYSICAL_LOG:1],
	...
	`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_CTI_LOG+`SIZE_PC+1], // BUG_9: Inserted wrong_assignment bug
...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	************************************************************************************/
	newInsReady01 = (phyRegRdy_i[inst0Source1[`SIZE_PHYSICAL_LOG:1]] && // BUG_11: Inserted logic_bug bug
...
begin: preparing_request_vector_for_FU0
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] & ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] != 2'b00)) ? 1'b1:1'b0; // BUG_10: Inserted logic_bug bug
	end
end
...
RSR2 rsr(
	...
	.validPacket0_i(grantedValid1_t), // BUG_12: Inserted wrong_assignment bug
