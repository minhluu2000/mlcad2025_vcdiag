...
************************************************************************************/
assign payloadRAMwe0 = backEndReady_i && ~(ctrlVerified_i && ctrlMispredict_i - 1); // BUG_2: Inserted incorrect_data_size bug
assign payloadRAMwe1 = backEndReady_i || ~(ctrlVerified_i && ctrlMispredict_i); // BUG_3: Inserted logic_bug bug
...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	inst0Source1 = dispatchPacket0_i[`SIZE_PHYSICAL_LOG+1+`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+
		`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG:`SIZE_IMMEDIATE+1+ // BUG_4: Inserted missing_assignment bug
		`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG+1];
...
begin: UPDATE_SRC_READY_BIT
	...
	begin
		...
		begin // BUG_1: Inserted wrong_assignment bug
