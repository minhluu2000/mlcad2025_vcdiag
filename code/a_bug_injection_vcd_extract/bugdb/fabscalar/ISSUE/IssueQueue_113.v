...
************************************************************************************/
assign payloadRAMwe1 = backEndReady_i && ~(ctrlVerified_i && ctrlMispredict_i); // BUG_5: Inserted wrong_assignment bug
...
assign CAM1we0 = backEndReady_i || ~(ctrlVerified_i && ctrlMispredict_i); // BUG_8: Inserted bitwise_corruption bug
...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	inst1Source2 = dispatchPacket1_i[2*`SIZE_PHYSICAL_LOG+2+`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+
		...
		`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+
		`SIZE_CTI_LOG+1];
inst2Source1 = dispatchPacket2_i[`SIZE_PHYSICAL_LOG+1+`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+ `INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG:`SIZE_IMMEDIATE+1+ `LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG+1] != 0; // BUG_7: Inserted logic_bug bug
...
begin: PREPARE_VALID_ARRAY_MISPRED
	...
	for(i=0; i<`SIZE_ISSUEQ; i=i+1)
	begin
		if(ctrlVerified_i || ctrlMispredict_i)    /* Unnecessary logic? */ // BUG_6: Inserted logic_bug bug
		begin
