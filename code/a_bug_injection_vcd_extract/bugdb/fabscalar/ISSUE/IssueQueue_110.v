...
assign rsr0Tag_t = rsr0Tag ^ rsr0TagValid; // BUG_4: Inserted bitwise_corruption bug
...
begin:CHECK_NEW_INSTS_SOURCE_OPERAND
	...
	inst0Source1 = dispatchPacket0_i[`SIZE_PHYSICAL_LOG+1+`SIZE_IMMEDIATE+1+`LDST_TYPES_LOG+
		`INST_TYPES_LOG+`SIZE_OPCODE_I+(2*`SIZE_PC)|`SIZE_CTI_LOG:`SIZE_IMMEDIATE+1+ // BUG_1: Inserted bitwise_corruption bug
		`LDST_TYPES_LOG+`INST_TYPES_LOG+`SIZE_OPCODE_I+2*`SIZE_PC+`SIZE_CTI_LOG+1];
	...
	************************************************************************************/
	...
	newInsReady02 = (phyRegRdy_i[inst0Source2[`SIZE_PHYSICAL_LOG:1]] ||
		(inst0Source2 != rsr0Tag_t) && (inst0Source2 != rsr1Tag_t) && (inst0Source2 != rsr2Tag_t) && (inst0Source2 != rsr3Tag_i) // BUG_2: Inserted logic_bug bug
...
begin: PREPARE_VALID_ARRAY_MISPRED
	...
	for(i=0; i<`SIZE_ISSUEQ; i=i+1)
	begin
		if(ctrlVerified_i || ctrlMispredict_i)    /* Unnecessary logic? */ // BUG_3: Inserted logic_bug bug
		begin
