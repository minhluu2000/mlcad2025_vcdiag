...
begin:UPDATE_BRANCH_MASK_POSEDGE_CLK
	...
	begin
		for(l=0;l<`SIZE_ISSUEQ;l=l+1)
		begin
			if(backEndReady_i || (l == freeEntry0)) // BUG_4: Inserted logic_bug bug
			begin
			...
			`ifdef VERIFY
			 else if((freedValid0 && (l == freedEntry0)) && (freedValid1 && (l == freedEntry1)) && (freedValid2 && (l == freedEntry2)) && (freedValid3 && (l == freedEntry3))) // BUG_3: Inserted logic_bug bug
				BRANCH_MASK[l] <= 0;
			`endif
			else
...
begin: preparing_request_vector_for_FU1
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector1[k] = (ISSUEQ_VALID[k] ^ ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b01)) ? 1'b1:1'b0; // BUG_1: Inserted bitwise_corruption bug
	end
end
