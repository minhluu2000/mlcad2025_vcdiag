...
begin: preparing_request_vector_for_FU0
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] | SRC0_REG_VALID[k] | SRC1_REG_VALID[k] | (ISSUEQ_FU[k] == 2'b00)) ? 1'b1:1'b0; // BUG_4: Inserted bitwise_corruption bug
	end
end
...
begin: preparing_request_vector_for_FU1
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector1[k] = (ISSUEQ_VALID[k] ^ ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b01)) ? 1'b1:1'b0; // BUG_1: Inserted logic_bug bug
	end
end
...
begin: preparing_request_vector_for_FU2
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector2[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] | SRC0_REG_VALID[k] | SRC1_REG_VALID[k] | (ISSUEQ_FU[k] == 2'b10)) ? 1'b1:1'b0; // BUG_3: Inserted bitwise_corruption bug
	end
end
