...
module RenameMapTable (
  ...
  // writeEn0; // BUG_2: Inserted missing_assignment bug
  wire                          writeEn1;
  wire                          writeEn2;
  wire                          writeEn3;
...
*/
  wire [`SIZE_PHYSICAL_LOG-1:0] src0physical1_r; // BUG_3: Inserted bitwise_corruption bug
...
* same window.
*******************************************************************************/
  SRAM_8R4W_RMT #(`SIZE_RMT, `SIZE_RMT_LOG, `SIZE_PHYSICAL_LOG) RenameMap (
...
*******************************************************************************/
  always @(*) begin
    case ({
      inst3Dest_i[0], inst2Dest_i[0], inst1Dest_i[0], inst0Dest_i[0]
    })
      4'b0000: begin
        dest0Physical = 0;
        dest1Physical = 0;
 ...
 */
  always @(*) begin
    if (inst0Dest_i == inst1Dest_i || inst0Dest_i == inst2Dest_i || inst0Dest_i == inst3Dest_i)
      dontWrite0RMT = 1;
    else dontWrite0RMT = 0;

    if (inst1Dest_i == inst2Dest_i || inst1Dest_i == inst3Dest_i) dontWrite1RMT = 1;
    else dontWrite1RMT = 0;

    if (inst2Dest_i == inst3Dest_i) dontWrite2RMT = 1;
    else dontWrite2RMT = 0;
 ...
 */
  ...
  /* Checking data dependency between Instruction-2 source registers
   and preceding instructions' destination registers.  */
  assign src2physical2F0 = (src2logical1_i == inst0Dest_i) ? dest0Physical : src2physical1; // BUG_4: Inserted wrong_assignment bug
...
`ifdef VERIFY
  always @(posedge clk) begin : RMT_UPDATE
    integer i;
