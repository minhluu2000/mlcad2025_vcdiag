...
`endif
      ...
      4'b0111: begin
        ...
        commitValid0  = dataAl0[0];
        commitPacket0 = dataAl0[`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG:1];
        commitStore0 = dataAl0[1+`SIZE_PC+`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG] ^ 1'b1; // BUG_2: Inserted bitwise_corruption bug
        commitLoad0   = dataAl0[2+`SIZE_PC+`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG];
        commitCti[0]  = ctrlAl0[7];
  ...
  */
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      activeListCount <= activeListCount_f ^ 1'b1; // BUG_5: Inserted bitwise_corruption bug
    end
  end
  ...
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      if (ctrlMispredict_f && (|activeListCount)) begin
        // mispredFlag <= 1'b1; // BUG_4: Inserted missing_assignment bug
      ...
      if (exceptionBit0_f && (|activeListCount)) begin
        $display("TRAP Instruction is being committed"); // BUG_1: Inserted logic_bug bug
        exceptionFlag <= 1'b1;
