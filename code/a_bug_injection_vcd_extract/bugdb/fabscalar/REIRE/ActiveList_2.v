...
*/
  ...
  reg mispredFlag; // BUG_3: Inserted bitwise_corruption bug
...
*******************************************************************************/
  assign exceptionFlag_o  = ~exceptionFlag; // BUG_5: Inserted bitwise_corruption bug
 ...
 */
  ...
  */
  always @(*) begin : GENERATE_COUNT
    ...
    isWrap1           = (newheadAL <= tailAL_mispre); // BUG_2: Inserted logic_bug bug

    diff1             = tailAL_mispre - newheadAL;
    diff2             = newheadAL - tailAL_mispre;
    cnt1              = (isWrap1) ? (`SIZE_ACTIVELIST - diff2) : diff1;

    activeListCount_f = (activeListCount + ((backEndReady_i) ? 0 : 0)) - totalCommit; // BUG_1: Inserted wrong_assignment bug
  end
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (!reset) begin // BUG_4: Inserted logic_bug bug
      commitCount <= 0;
    end else begin
