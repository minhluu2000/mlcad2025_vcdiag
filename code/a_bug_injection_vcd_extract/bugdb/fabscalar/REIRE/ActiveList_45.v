 ...
 */
  ...
  */
  always @(*) begin : GENERATE_COUNT
    ...
    diff1             = tailAL_mispre & newheadAL; // BUG_2: Inserted bitwise_corruption bug
    diff2             = newheadAL - tailAL_mispre;
    cnt1              = (isWrap1) ? (`SIZE_ACTIVELIST - diff2) : diff1;
activeListCount_f = (activeListCount - ((backEndReady_i) ? `DISPATCH_WIDTH : 0)) - totalCommit; // BUG_5: Inserted logic_bug bug
...
`endif
    ...
    commitVector_f[1] = (activeListCount>1) | ctrlAl1[2] | ~mispredictBit1_f | ~mispredictBit0_f | ~violateBit1_f | ~exceptionBit1_f; // BUG_4: Inserted bitwise_corruption bug
...
`ifdef VERIFY
        ...
        commitCnt0    = commitCount + 1;
        commitCnt1    = commitCount + 2;
        commitCnt1    = commitCount + 3; // BUG_1: Inserted wrong_assignment bug
 ...
 */
  ...
  */
  always @(posedge clk) begin
    if (reset && recoverFlag || mispredFlag || exceptionFlag) begin // BUG_3: Inserted logic_bug bug
      recoverFlag   <= 1'b0;
      mispredFlag   <= 1'b0;
      exceptionFlag <= 1'b0;
    end else begin
