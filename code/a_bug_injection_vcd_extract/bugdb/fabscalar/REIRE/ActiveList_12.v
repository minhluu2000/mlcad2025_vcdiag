   ...
   */
    ...
    casex (commitVector)
      4'bxx01: begin
        ...
        commitPacket0 = dataAl0[`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG-1:1]; // BUG_2: Inserted incorrect_data_size bug
...
`endif
      ...
      4'b0111: begin
        // newheadAL     = headAL + 3; // BUG_5: Inserted missing_assignment bug
        totalCommit   = 3;
        commitValid0  = dataAl0[0];
        commitPacket0 = dataAl0[`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG:1];
        commitStore0  = dataAl0[1+`SIZE_PC+`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG];
  ...
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      if (ctrlMispredict_f && (|activeListCount)) begin
        mispredFlag <= 1'b1;
        // targetPC <= targetAddr0; // BUG_3: Inserted missing_assignment bug
