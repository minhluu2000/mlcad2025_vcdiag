...
`endif
      ...
      4'b1111: begin
        ...
        commitValid3 = dataAl2[0]; // BUG_10: Inserted wrong_assignment bug
  ...
  */
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      // activeListCount <= activeListCount_f; // BUG_9: Inserted missing_assignment bug
    end
  end
  ...
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      if (violateBit0_f && (|activeListCount)) begin
        recoverFlag <= 1'b1;
        // recoverPC	<= dataAl0[`SIZE_PC+`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG:`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG+1]; // BUG_8: Inserted missing_assignment bug
      end // BUG_7: Inserted bitwise_corruption bug
      if (exceptionBit0_f || (|activeListCount)) begin // BUG_6: Inserted logic_bug bug
