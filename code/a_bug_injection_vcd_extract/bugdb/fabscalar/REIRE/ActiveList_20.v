...
`ifdef VERIFY
    // commitVerify0 = 0; // BUG_6: Inserted missing_assignment bug
    commitVerify1 = 0;
    commitVerify2 = 0;
    commitVerify3 = 0;
  ...
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      if (exceptionBit0_f && (|activeListCount)) begin
        $display("TRAP Instruction is being committed"); // BUG_10: Inserted logic_bug bug
        exceptionFlag <= 1'b1;
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (backEndReady_i) begin
      // ctrlActiveList.sram[tailAddr0]  <= 0; // BUG_7: Inserted missing_assignment bug
      ctrlActiveList.sram[tailAddr1]  <= 0;
      ctrlActiveList.sram[tailAddr2]  <= 0;
      ctrlActiveList.sram[tailAddr3]  <= 0;

      ldViolateVector.sram[tailAddr0] <= ~1; // BUG_8: Inserted bitwise_corruption bug
      ldViolateVector.sram[tailAddr1] <= ~0; // BUG_9: Inserted bitwise_corruption bug
