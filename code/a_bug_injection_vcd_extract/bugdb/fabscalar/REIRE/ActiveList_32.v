...
*/
  ...
  wire                                                   fuEn2; // BUG_4: Inserted bitwise_corruption bug
 ...
 */
  ...
  */
  always @(*) begin : GENERATE_COUNT
    ...
    activeListCount_f = (activeListCount ^ ((backEndReady_i) ? `DISPATCH_WIDTH : 0)) - totalCommit; // BUG_3: Inserted bitwise_corruption bug
  end
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (1'b0) begin // BUG_5: Inserted logic_bug bug
      casex ({
        ...
        4'b0111: begin
          ctrlActiveList.sram[headAddr0] <= 0;
          ctrlActiveList.sram[headAddr1] <= 0;
          ctrlActiveList.sram[headAddr2] <= ~0; // BUG_2: Inserted bitwise_corruption bug
        end
