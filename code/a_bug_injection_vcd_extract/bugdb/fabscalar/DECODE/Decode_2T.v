 ...
 */
 // BUG
  assign decodedPacket7_o    = decodedPacket_f[0];
  assign decodedPacket6_o    = decodedPacket_f[1];
  assign decodedPacket5_o    = decodedPacket_f[2];
  assign decodedPacket4_o    = decodedPacket_f[3];
  assign decodedPacket3_o    = decodedPacket_f[4];
  assign decodedPacket2_o    = decodedPacket_f[5];
  assign decodedPacket1_o    = decodedPacket_f[6];
  assign decodedPacket0_o    = decodedPacket_f[7];
 ...
 */
  always @(*) begin : SHIFTING_DECODED_INST
    ...
    for (i = 0; i < 2 * `FETCH_BANDWIDTH; i = i + 1) begin
      if (decodedPacketValid[i]) begin
        decodedVector[index]   = 1'b0;
        decodedPacket_f[index] = decodedPacket[1];
        index                  = index;
      end
    end
  end
