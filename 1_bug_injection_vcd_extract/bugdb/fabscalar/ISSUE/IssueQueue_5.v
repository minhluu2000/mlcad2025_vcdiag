...
begin: preparing_request_vector_for_FU2
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector2[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b10)) ? 1'b1:1'b0; // BUG_9: Inserted logic_bug bug
	end
end
 ...
 * *************************/
	...
	assign granted1Dest  = grantedPacket1_t[`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+`LDST_TYPES_LOG+
		// `SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG:`SIZE_IMMEDIATE+ // BUG_11: Inserted missing_assignment bug
		`LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG+1];
	...
	assign granted0Entry = grantedPacket0_t[`SIZE_ISSUEQ_LOG+2*`SIZE_PHYSICAL_LOG+`SIZE_LSQ_LOG+
		...
		`SIZE_ACTIVELIST_LOG+`CHECKPOINTS_LOG+`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+
		`LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG+1]; // BUG_12: Inserted bitwise_corruption bug
