...
SRAM_4R4W_PAYLOAD #(`SIZE_ISSUEQ,`SIZE_ISSUEQ_LOG,`SIZE_PAYLOAD_WIDTH) payloadRAM(.clk(clk), // BUG_1: Inserted logic_bug bug
	.reset(reset),
...
************************************************************************************/
assign src0RegValid_t0 = ISSUEQ_VALID | (~ISSUEQ_SCHEDULED) & (SRC0_REG_VALID | src0_matchLines0 | src0_matchLines1 | src0_matchLines2 | src0_matchLines3); // BUG_2: Inserted bitwise_corruption bug
...
begin: preparing_request_vector_for_FU0
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b00)) ? 1'b1:1'b0; // BUG_3: Inserted bitwise_corruption bug
	end
end
...
Select select0(.clk(clk),
	...
	.grantedEntry_o(grantedEntry1), // BUG_4: Inserted wrong_assignment bug
