...
begin: PREPARE_VALID_ARRAY_NORMAL
	...
	begin
		if((freedValid0 && k == freedEntry0) || (freedValid1 && k == freedEntry1) || (freedValid2 && k == freedEntry2) || (freedValid3 && k == freedEntry3))
			...
			issueqValid_normal[k] = ~issueqValid_tmp[k]; // BUG_1: Inserted bitwise_corruption bug
	end
end
...
begin: PREPARE_VALID_ARRAY_MISPRED
	...
	for(i=0; i<`SIZE_ISSUEQ; i=i+1)
	begin
		if(ctrlVerified_i || ctrlMispredict_i)    /* Unnecessary logic? */ // BUG_2: Inserted logic_bug bug
		begin
...
Select select2(.clk(clk),
	.reset(reset),
	.requestVector_i(requestVector1), // BUG_4: Inserted wrong_assignment bug
