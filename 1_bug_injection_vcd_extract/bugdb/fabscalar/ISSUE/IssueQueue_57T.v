...
always @(posedge clk)
begin
	if(reset) // BUG_1: Inserted wrong_assignment bug
	begin
		ISSUEQ_VALID <= 0;
		ISSUEQ_SCHEDULED <= 0;
	end
...
always @(*)
begin: preparing_request_vector_for_FU0 // BUG_3: Inserted missing_assignment bug
	integer k;
...
begin: preparing_request_vector_for_FU1
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector1[k] = (ISSUEQ_VALID[k] ^ ~ISSUEQ_SCHEDULED[k] & SRC0_REG_VALID[k] & SRC1_REG_VALID[k] & (ISSUEQ_FU[k] == 2'b01)) ? 1'b1:1'b0; // BUG_2: Inserted bitwise_corruption bug
	end
end
