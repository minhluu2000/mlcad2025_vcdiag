...
************************************************************************************/
assign payloadRAMwe0 = backEndReady_i && (ctrlVerified_i ^ ctrlMispredict_i); // BUG_1: Inserted bitwise_corruption bug
assign payloadRAMwe1 = backEndReady_i && ~(ctrlVerified_i && ctrlMispredict_i);
assign payloadRAMwe2 = backEndReady_i && (ctrlVerified_i || ctrlMispredict_i); // BUG_3: Inserted bitwise_corruption bug
 ...
 * *************************/
	...
	assign granted1Entry = grantedPacket1_t[`SIZE_ISSUEQ_LOG+2*`SIZE_PHYSICAL_LOG+`SIZE_LSQ_LOG+
		...
		`LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG:2*`SIZE_PHYSICAL_LOG+`SIZE_LSQ_LOG+
		`SIZE_ACTIVELIST_LOG+`CHECKPOINTS_LOG+`SIZE_PHYSICAL_LOG+`SIZE_IMMEDIATE+
		`LDST_TYPES_LOG+`SIZE_OPCODE_I+`SIZE_PC+`SIZE_PC+`SIZE_CTI_LOG+1]; // BUG_4: Inserted wrong_assignment bug
