...
begin: preparing_request_vector_for_FU0
	...
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
		requestVector0[k] = (ISSUEQ_VALID[k] | ~ISSUEQ_SCHEDULED[k] | SRC0_REG_VALID[k] | SRC1_REG_VALID[k] | (ISSUEQ_FU[k] == 2'b00)) ? 1'b1:1'b0; // BUG_1: Inserted bitwise_corruption bug
	end
end
...
Select select0(.clk(clk),
	...
	.grantedValid_o(grantedValid1) // BUG_4: Inserted wrong_assignment bug
);
...
begin: preparing_request_vector_for_FU2
	integer k; // BUG_2: Inserted bitwise_corruption bug
	for(k=0; k<`SIZE_ISSUEQ; k=k+1)
	begin
