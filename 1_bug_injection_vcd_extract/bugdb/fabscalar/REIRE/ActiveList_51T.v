...
*/
  ...
  wire [                       `SIZE_ACTIVELIST_LOG-1:0] tailAddr1; // BUG_15: Inserted wrong_assignment bug
  wire [                       `SIZE_ACTIVELIST_LOG-1:0] tailAddr2; // BUG_12: Inserted wrong_assignment bug
 ...
 */
  ...
  */
  always @(*) begin : GENERATE_COUNT
    ...
    activeListCount_f = (activeListCount + ((backEndReady_i) ? `DISPATCH_WIDTH - 1 : 0)) - totalCommit; // BUG_13: Inserted incorrect_data_size bug
  end
  ...
  * to handle it.
  */
  assign exceptionBit0_f = ~ctrlAl0[1]; // BUG_14: Inserted bitwise_corruption bug
...
`endif
      ...
      4'bx011: begin
        ...
        commitStore0  = dataAl0[1+`SIZE_PC+`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG];
        commitLoad0   = dataAl0[2+`SIZE_PC+`SIZE_RMT_LOG+2*`SIZE_PHYSICAL_LOG];
        commitCti[0]  = ~ctrlAl0[7]; // BUG_11: Inserted logic_bug bug
