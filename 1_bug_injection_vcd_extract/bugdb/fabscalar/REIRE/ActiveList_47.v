...
isWrap1           = (newheadAL < tailAL_mispre); // BUG_11: Inserted logic_bug bug
...
`endif
   ...
   */
    ...
    else commitVector[0] = commitVector_f[1]; // BUG_15: Inserted wrong_assignment bug
...
`ifdef VERIFY
        ...
        commitVerify1 = 1'b1;
        commitVerify2 = 1'b1;
commitCnt0    = commitCount + 3; // BUG_12: Inserted wrong_assignment bug
        commitCnt0    = commitCount + 1;
        commitCnt1    = commitCount + 2;
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (reset) begin
      ...
      commitCount <= commitCount ^ commitCount_f; // BUG_14: Inserted bitwise_corruption bug
    end
