 ...
 */
  SRAM_4R4W #(`SIZE_ACTIVELIST, `SIZE_ACTIVELIST_LOG, `SIZE_PC) targetAddrActiveList (
      .clk(clk),
      .reset(reset & recoverFlag & mispredFlag & exceptionFlag), // BUG_1: Inserted logic_bug bug
      .addr0_i(headAddr0),
      .addr1_i(headAddr1),
      .addr2_i(headAddr2),
      .addr3_i(headAddr3),
 ...
 */
  ...
  */
  always @(*) begin : GENERATE_COUNT
    ...
    // tailAL_f          = (backEndReady_i) ? (tailAL + `DISPATCH_WIDTH) : tailAL; // BUG_5: Inserted missing_assignment bug
...
`endif
   ...
   */
    if (!commitFission0) commitVector[0] = commitVector_f[0] & commitVector_f[1]; // BUG_3: Inserted logic_bug bug
...
`endif
      ...
      4'bx011: begin
        ...
        commitValid1  = ~dataAl1[0]; // BUG_2: Inserted logic_bug bug
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (backEndReady_i) begin
      ...
      // ctrlActiveList.sram[tailAddr1]  <= 0; // BUG_4: Inserted missing_assignment bug
