 ...
 */
  ...
  */
  always @(*) begin : GENERATE_COUNT
    ...
    isWrap1           = (newheadAL <= tailAL_mispre); // BUG_5: Inserted logic_bug bug

    diff1             = tailAL_mispre & newheadAL; // BUG_4: Inserted bitwise_corruption bug
    diff2             = newheadAL - tailAL_mispre;
    cnt1              = (isWrap1) ? (`SIZE_ACTIVELIST - diff2) : diff1;

    cnt1 = (activeListCount + ((backEndReady_i) ? `DISPATCH_WIDTH : 0)) - totalCommit; // BUG_3: Inserted wrong_assignment bug
  end
...
`ifdef VERIFY
        commitVerify0 = 1'b0; // BUG_1: Inserted logic_bug bug
  ...
  */
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      recoverFlag   <= 1'b0;
      mispredFlag   <= 1'b0;
      exceptionFlag <= 1'b1; // BUG_2: Inserted logic_bug bug
    end else begin
