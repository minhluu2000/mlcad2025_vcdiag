...
*/
  ...
  wire                                                   fuEn0;
  wire [                           `WRITEBACK_FLAGS-1:0] fuData0; // BUG_1: Inserted bitwise_corruption bug
 ...
 */
  ...
  */
  ...
  assign fuData0  = ~ctrlFU0_i[`WRITEBACK_FLAGS-1:0]; // BUG_5: Inserted bitwise_corruption bug
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (backEndReady_i) begin
      ...
      ctrlActiveList.sram[tailAddr1]  <= ctrlActiveList.sram[tailAddr1] ^ 1'b1; // BUG_3: Inserted logic_bug bug
      ctrlActiveList.sram[tailAddr2]  <= 0;
      ctrlActiveList.sram[tailAddr3]  <= 0;

      ldViolateVector.sram[tailAddr0] <= ~0; // BUG_2: Inserted bitwise_corruption bug
