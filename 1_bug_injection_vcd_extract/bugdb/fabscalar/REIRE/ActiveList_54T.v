...
*/
  ...
  reg                                                    commitStore3;
  reg                                                    commitLoad3; // BUG_4: Inserted bitwise_corruption bug
  reg                                                    commitFission3;
...
************************************************************************************/
  ...
  SRAM_4R4W #(`SIZE_ACTIVELIST, `SIZE_ACTIVELIST_LOG, `WRITEBACK_FLAGS) ctrlActiveList (
      .clk(clk),
      .reset(reset ^ violateBit0_f ^ ctrlMispredict_f ^ exceptionBit0_f), // BUG_2: Inserted bitwise_corruption bug
      .addr0_i(headAddr0),
      .addr1_i(headAddr1),
      .addr2_i(headAddr2),
      .addr3_i(headAddr3),
...
`endif
    ...
    commitVector_f[0] = (activeListCount > 0) & ctrlAl0[2] & ~violateBit0_f & ~violateBit0_f; // BUG_5: Inserted wrong_assignment bug
  ...
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      if (ctrlMispredict_f && (|activeListCount)) begin
        // mispredFlag <= 1'b1; // BUG_3: Inserted missing_assignment bug
