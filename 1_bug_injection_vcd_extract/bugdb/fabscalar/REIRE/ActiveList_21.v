...
`endif
    ...
    commitVector_f[1] = (activeListCount>1) | ctrlAl1[2] & ~mispredictBit1_f & ~mispredictBit0_f & ~violateBit1_f & ~exceptionBit1_f; // BUG_3: Inserted logic_bug bug
  ...
  */
  always @(posedge clk) begin
    if (reset || recoverFlag || mispredFlag || exceptionFlag) begin
      ...
      activeListCount <= activeListCount; // BUG_2: Inserted wrong_assignment bug
    end
  end
...
`ifdef VERIFY
  always @(posedge clk) begin : CLEAR_CTRL_AL
    ...
    if (backEndReady_i) begin
      ...
      ctrlActiveList.sram[tailAddr1]  <= ~0; // BUG_5: Inserted bitwise_corruption bug
      ...
      ldViolateVector.sram[tailAddr2] <= 1; // BUG_4: Inserted wrong_assignment bug
